...
<head>
	...
	<script type="text/javascript" src="${pageContext.request.contextPath }/customize/js/tool.js"></script>
 	...
 	$(function(){
      ...
      //部门下拉框改变事件
      $("[name='posiAdju.positionByAfter.department.id']").change(function(){
      	/* 异步获得职位 */
       	var did = $(this).val();
       	position(did,function(data){
       		var target = $("[name='posiAdju.positionByAfter.id']");
  ...
  <body>
	...
	<div class="container-fluid mybody">
	      ...
	      <div class="container-fluid well mycontext" id="container-Info">
	        ...
	        <form method="post" action="${pageContext.request.contextPath }/pers/findPosiAdjuInfoInPaaOf.html">
		      	<!-- 页标 -->
	          ...
	          <div class="form-group form-inline text-center col-md-offset-1 col-md-10">
	            <div class="form-group col-md-4">
	                ...
	                <input type="text" class="form-control" placeholder="Like Position Name" name="posiAdju.positionByAfter.name" default="${posiAdju.positionByAfter.name }">
	            </div>
	            <div class="form-group col-md-4">
	                <label class="control-label">部 门：</label>
	                <select class="form-control" style="width:196px;" name="posiAdju.positionByAfter.department.id" sele="${posiAdju.positionByAfter.department.id }">
	                  <option value="-1">全部</option>
	                </select>
	            </div>
	            <script type="text/javascript">
	            	department(function(data){
	            		var target = $("[name='posiAdju.positionByAfter.department.id']");
				   		for(var idx in data){
				   			var dept = data[idx];
	            ...
	            <div class="form-group col-md-4">
	                ...
	                <select class="form-control" style="width:196px;" name="posiAdju.positionByAfter.id" sele="${posiAdju.positionByAfter.id }">
	                  <option value="-1">全部</option>
	                </select>
	            </div>
	            <script type="text/javascript">
	            	/* 异步获得职位 */
	            	var did = $("[name='posiAdju.positionByAfter.department.id']").attr("sele");
	            	position(did,function(data){
	            		var target = $("[name='posiAdju.positionByAfter.id']");
				   		for(var idx in data){
				   			var posi = data[idx];
	          ...
	          <div class="form-group form-inline text-center col-md-offset-1 col-md-10">
	            <div class="form-group col-md-6">
	                ...
	                <input type="text" class="form-control" placeholder="Like Staff No" name="posiAdju.staff.no" default="${posiAdju.staff.no }">
	            </div>
	            <div class="form-group col-md-6">
	                <label class="control-label">姓 名：</label>
	                <input type="text" class="form-control" placeholder="Like Staff Name" name="posiAdju.staff.name" default="${posiAdju.staff.name }">
	            </div>
	          </div>
	          <div class="form-group form-inline text-center col-md-offset-1 col-md-10">
	          ...
	          <table class="ui blue selectable celled table">
	            ...
	            <tbody>
	              <c:forEach items="${posiAdjus }" var="tpa">
	              	<tr>
		                <td>${tpa.positionByAfter.department.name }</td>
		                <td>${tpa.positionByAfter.name }</td>
		                <td ><fmt:formatDate pattern="yyyy-MM-dd" value="${tpa.date}" /></td>
		                <td>${tpa.staff.name }</td>
		                <td>${tpa.discription }</td>
